...
import ViewVideos from "./viewVideoHome.jsx";
...
const Playlist = () => {
  ...
  useEffect(() => {
    console.log(videos);
    console.log("From useEffect:", playingVideo);
  }, [videos, playingVideo]);
  return (
    <>
          ...
          >
            <div className="videoUrl" onClick={() => setPlayingVideo(video)}>
              ...
              {video.url ? (
                ...
                <AiOutlineLoading3Quarters className="loadingVid" size="4em" />
              )}
            </div>
            <div
              className="videoInfo text-center"
              onClick={() => setPlayingVideo(video)}
              style={{ width: "100%" }}
            >
              <b>{video.title}</b>
...
const Subscriptions = () => {
  ...
  useEffect(() => {
    ...
    const effect = async () => {
      var videosArr = [];
      const dbQuery = query(collection(firestoreObj, "videos"), limit(10));
      ...
      videosArr.map(async (video) => {
        const videosRef = ref(storage, "videos/" + video.storageName);
