        ...
        <div className='body '>
            ...
            <IonCard className='text-center p-2' id='devotionCard'>
                <div className='devoPic'>
                    <img src={dailyDevotion.image} alt="Devotional picture" /><br />
                </div>
            ...
            <div className=''>
                { videos && videos.map((video) =>{
                    return(
                        <IonCard className='sermon'>
                            {/* get video from database */}
                            <video controls={true} src={video.url}
                                onMouseEnter={(e) => {startPreview(e);}}
                                onMouseLeave={(e) =>{stopPreview(e)}}>
                            </video>
                            <IonCardSubtitle ><b>{video.title}</b></IonCardSubtitle>
                            <IonCardSubtitle >{video.author}</IonCardSubtitle>
                            <IonButton color='danger' onClick={() => {handlePlay(video)}} >Play<FiPlayCircle /></IonButton>
                        </IonCard>
                    )
                })}
            </div>
